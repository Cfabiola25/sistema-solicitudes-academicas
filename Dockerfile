...
# Instalar Apache Ant de forma limpia y minimizar la cache de apt
RUN apt-get update \
...
# Copiar el WAR generado (nombre explícito según nbproject/project.properties)
COPY --from=builder /app/dist/solicitudes-academicas.war $CATALINA_HOME/webapps/ROOT.war

# No incluir credenciales en la imagen en producción. Montar en tiempo de ejecución:
# ej: docker run -v /host/database.properties:$CATALINA_HOME/lib/database.properties ...
