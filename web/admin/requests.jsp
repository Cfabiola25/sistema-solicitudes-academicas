    ...
    <div>
        ...
        <nav class="flex flex-col gap-1">
            ...
            <a href="<%=request.getContextPath()%>/admin/requests"
               class="nav-link active flex items-center gap-3 px-3 py-2.5 text-sm font-medium text-gray-400 transition-all">
                <i class="fa-regular fa-folder-open text-sm w-4 text-center"></i> Solicitudes
            </a>
    ...
    <div class="p-8 flex-1">
        ...
        <div class="flex flex-col md:flex-row items-start md:items-end justify-between mb-6 gap-4">
            <div>
                ...
                <p class="text-sm text-gray-400 mt-1">Gestión y revisión de requerimientos estudiantiles.</p>
            </div>
            
            <form method="get" class="flex items-center gap-3 bg-white p-2 rounded-xl shadow-sm border border-gray-100">
                ...
                <select name="state" class="text-sm font-medium text-gray-700 bg-gray-50 border-none rounded-lg py-1.5 px-3 focus:ring-2 focus:ring-[#c8102e]/30 cursor-pointer outline-none">
                    <option value="" <%= request.getParameter("state") == null || request.getParameter("state").isEmpty() ? "selected" : "" %>>Todos los estados</option>
                        ...
                        <tr class="border-b border-gray-50 hover:bg-gray-50/60 transition-colors">
                            ...
                            <td class="py-4 px-2">
                                ...
                                <% if ("Pendiente".equals(st)) { %>
                                    <span class="px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider rounded-lg bg-amber-50 text-amber-600 border border-amber-100"><i class="fa-solid fa-clock mr-1"></i> <%= st %></span>
                                <% } else if ("Aprobada".equals(st)) { %>
                                    <span class="px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider rounded-lg bg-green-50 text-green-600 border border-green-100"><i class="fa-solid fa-check mr-1"></i> <%= st %></span>
                                <% } else if ("Rechazada".equals(st)) { %>
                                    <span class="px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider rounded-lg bg-red-50 text-red-600 border border-red-100"><i class="fa-solid fa-xmark mr-1"></i> <%= st %></span>
                                <% } else if ("Enviada".equals(st)) { %>
                                    <span class="px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider rounded-lg bg-blue-50 text-blue-600 border border-blue-100"><i class="fa-solid fa-paper-plane mr-1"></i> <%= st %></span>
                                <% } else { %>
                                    <span class="px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider rounded-lg bg-gray-100 text-gray-600"><%= st %></span>
                                <% } %>
                            </td>
