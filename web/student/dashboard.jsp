...
<aside class="w-[220px] bg-white border-r border-gray-100 flex flex-col justify-between py-6 px-4 fixed h-full z-10">
    <div>
        ...
        <div class="flex items-center gap-3 bg-gray-50 rounded-xl p-3 mb-6">
            ...
            <div>
                <p class="text-xs font-bold text-gray-800 leading-none"><%= student.getNombreCompleto() %></p>
                <p class="text-[10px] text-gray-400 mt-0.5">Portal de Estudiante</p>
            </div>
        </div>
...
<main class="flex-1 ml-[220px] min-h-screen flex flex-col">
    <header class="bg-white border-b border-gray-100 px-8 py-4 flex items-center justify-between sticky top-0 z-10">
        <div>
            <p class="text-xs font-bold uppercase tracking-[0.2em] text-gray-400">Panel estudiantil</p>
            <h1 class="text-lg font-extrabold text-gray-900">Bienvenido, <%= student.getNombre() %></h1>
        </div>
        <div class="flex items-center gap-4">
            <a href="<%=request.getContextPath()%>/student/new-request" class="hidden sm:inline-flex items-center gap-2 rounded-xl bg-[#c8102e] px-4 py-2 text-sm font-bold text-white shadow-sm shadow-red-600/20 hover:bg-red-700 transition-colors">
                <i class="fa-solid fa-plus"></i> Nueva solicitud
            </a>
            <div class="w-9 h-9 rounded-full bg-[#c8102e]/10 flex items-center justify-center">
                <i class="fa-solid fa-user-graduate text-[#c8102e] text-sm"></i>
            </div>
        </div>
    </header>
    ...
    <div class="p-8 flex-1">
        <div class="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Total solicitudes</p>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            <div class="xl:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <div class="flex items-start justify-between mb-5">
                    <div>
                        <h2 class="text-xl font-extrabold text-gray-900">Solicitudes recientes</h2>
                        <p class="text-sm text-gray-400 mt-1">Seguimiento rápido de tus últimos movimientos.</p>
                    </div>
                    <a href="<%=request.getContextPath()%>/student/requests" class="text-sm font-bold text-[#c8102e] hover:underline">Ver todas</a>
                </div>
                <div class="overflow-x-auto">
                    <table class="min-w-full">
                        ...
                        <tbody>
                            <% if (recent.isEmpty()) { %>
                                <tr>
                                    <td colspan="5" class="py-10 text-center text-sm text-gray-400">Aún no tienes solicitudes registradas.</td>
                                </tr>
                            <% } %>
                            <% for (Solicitud sol : recent) { %>
                                <tr class="border-b border-gray-50 hover:bg-gray-50/60 transition-colors">
                                    <td class="py-4 pr-4 text-sm font-bold text-gray-500">#<%= sol.getId() %></td>
                                    <td class="py-4 pr-4 text-sm font-semibold text-gray-800"><%= sol.getTipo().getNombre() %></td>
                                    <td class="py-4 pr-4 text-sm text-gray-500"><%= sol.getFechaSolicitud() != null ? sol.getFechaSolicitud().toLocalDate().toString() : "" %></td>
                                    <td class="py-4 pr-4 text-sm">
                                        ...
                                        <% } else { %>
                                            <span class="inline-flex items-center rounded-lg bg-gray-100 px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider text-gray-600"><%= est %></span>
                                        <% } %>
                                    </td>
            ...
            <div class="space-y-6">
                <div class="bg-[#c8102e] rounded-2xl p-6 text-white shadow-lg shadow-red-500/10 relative overflow-hidden">
                    ...
                    <p class="text-xs font-bold uppercase tracking-[0.24em] text-white/70">Atención</p>
                    <h3 class="mt-2 text-2xl font-extrabold leading-tight">Usa el chat para resolver dudas antes del dictamen final.</h3>
                    <p class="mt-3 text-sm text-white/75">Cada solicitud ahora puede tener mensajes entre estudiante y administración.</p>
                    <a href="<%=request.getContextPath()%>/student/new-request" class="mt-5 inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-bold text-[#c8102e] hover:bg-red-50 transition-colors">
                        <i class="fa-solid fa-plus"></i> Crear solicitud
                    </a>
                </div>

                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    <h3 class="text-lg font-extrabold text-gray-900 mb-4">Información personal</h3>
                    <div class="space-y-3 text-sm">
                        <p><span class="font-semibold text-gray-500">Nombre:</span> <%= student.getNombreCompleto() %></p>
                        <p><span class="font-semibold text-gray-500">Correo:</span> <%= student.getEmail() %></p>
                        <p><span class="font-semibold text-gray-500">Programa ID:</span> <%= student.getProgramaId() != null ? student.getProgramaId() : "-" %></p>
                    </div>
                </div>
            </div>
        </div>
